	...
	if (! empty($mpcth_options['mpcth_socials']))
		foreach($mpcth_options['mpcth_socials'] as $name => $enable) {
			if($enable) {
				...
				echo '<li>';
					echo '<a href="' . ($prefix != '' ? $prefix . $mpcth_options['mpcth_social_' . $name] : esc_url($mpcth_options['mpcth_social_' . $name])) . '" class="mpcth-social-' . $name . '" ' . $target . '>';
		...
		<?php if ( sizeof( WC()->cart->get_cart() ) > 0 ) : ?>
			<ul class="mpcth-mini-cart-products">
				<?php foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) :
					...
					// Get variations
					$enable_minicart_variations = get_field( 'mpcth_' );
					$enable_minicart_variations = true;
					$variations_data = '';

					if( $enable_minicart_variations && !empty( $_product->variation_id ) )
